...
void MetricHeuristic::setGoal(GoalState& goal_state){
    ...
    m_yaw = goal_state.getSolnState()->robot_pose().getContBaseState().theta();
...
int MetricHeuristic::getGoalHeuristic(GraphStatePtr state){
    ...
    int cost = (m_yaw - state->robot_pose().getContBaseState().theta()) *(m_yaw - state->robot_pose().getContBaseState().theta()); 
